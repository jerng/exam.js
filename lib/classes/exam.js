    ...
    constructor ( data ) {
        ...
        if ( 'config' in data ) {
            if ( 'expand' in data.config ) {
                if ( 'initial_context' in data.config.expand ) {
                    this.config.expand.initial_context =
                        data.config.expand.initial_context 
                }
...
**`)
        ...
        let initial_context_label = 'Concerns: initial execution context; loop ran synchronously:'

        if ( this.config.expand.initial_context ) 
                { console.group           ( initial_context_label ) 
        } else  { console.groupCollapsed  ( initial_context_label ) }
...
*   Message : ${ e }`) 
        ...
        console.groupEnd ( initial_context_label )
...
**` )
            ...
            Promise.all ( concerns ).then ( fValues => {
                ...
                console.group ( `Concerns (${concerns.length}) : loop through asynchronous execution contexts:` )
                    ...
                    {
                        let warnings_label = `Warnings (${partitioned.warnings.length}):`
                        
                        if ( this.config.expand.warnings ) 
                                { console.group             ( warnings_label ) 
                        } else  { console.groupCollapsed    ( warnings_label ) }
                        
                        partitioned.warnings.forEach        ( ac => ac.render() )
                        console.groupEnd                    ( warnings_label ) 
                    }
                    
                    {
                        let tests_verifiable_label = `Machine Verifiable Tests (${partitioned.tests.verifiable.length}):`
                        
                        if ( this.config.expand.tests.verifiable ) 
                                { console.group             ( tests_verifiable_label ) 
                        } else  { console.groupCollapsed    ( tests_verifiable_label ) }
                        
                        partitioned.tests.verifiable.forEach( ac => ac.render() )

                        console.groupEnd                    ( tests_verifiable_label ) 
                    }

                    {
                        let tests_legibility_label = `Human Legibility Tests (${partitioned.tests.legibility.length}):`
                        
                        if ( this.config.expand.tests.legibility ) 
                                { console.group             ( tests_legibility_label ) 
                        } else  { console.groupCollapsed    ( tests_legibility_label ) }
                        
                        partitioned.tests.legibility.forEach( ac => ac.render() )

                        console.groupEnd                    ( tests_legibility_label ) 
                    }
